...
// NEW: a hammer prop the player can pick up (E) and drop (G) via the engine's
// held-item system (engine.pickupItem / engine.dropHeldItem — see engine.js).
...
export function createRoom1(scene, engine) {
  ...
  // room1's floor sits at world Y 0 — matches engine.floorY's default,
  // so dropped items rest flush with the ground without extra setup.
  engine.floorY = 0;
  ...
  // engine.pickupItem() parents it to the camera, engine.dropHeldItem() puts it
  // back in the scene wherever the player is standing when they drop it.
  const hammerMat = new THREE.MeshStandardMaterial({ color: 0x8a8378, roughness: 0.55, metalness: 0.6 });
  const hammerHandleMat = new THREE.MeshStandardMaterial({ color: 0x4a3016, roughness: 0.85 });

  const hammerGroup = new THREE.Group();
  const hammerHandle = new THREE.Mesh(new THREE.CylinderGeometry(0.018, 0.02, 0.32, 8), hammerHandleMat);
  hammerHandle.rotation.z = Math.PI / 2.1;
  hammerHandle.position.set(0, 0.05, 0);
  const hammerHead = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.05, 0.05), hammerMat);
  ...
  // resting on the floor near the puja corner, a little out from the wall
  hammerGroup.position.set(-ROOM_W / 2 + 0.9, 0, -2.2);
