...
export class Engine {
  constructor(canvas) {
    ...
    // ---------- noise system ----------
    // Lightweight pub/sub so enemies (e.g. vrishchikenemy.js) can react to
    // in-world sounds — thrown items landing, doors slamming, glass
    // breaking, footsteps on a loud surface, etc — without every enemy
    // needing its own listener wiring. Anything in the game can call
    // engine.emitNoise(position, {radius, loudness}) and any enemy that
    // subscribed via engine.onNoise(cb) gets notified if it's in range.
    this._noiseListeners = [];

    /**
     * Subscribe to noise events.
     * @param {(event: {position: THREE.Vector3, radius: number, loudness: number}) => void} callback
     * @returns {() => void} unsubscribe function
     */
    this.onNoise = (callback) => {
      this._noiseListeners.push(callback);
      return () => {
        this._noiseListeners = this._noiseListeners.filter((cb) => cb !== callback);
      };
    };

    /**
     * Broadcast a noise event to every subscriber within range.
     * @param {THREE.Vector3} position world-space origin of the sound
     * @param {{radius?: number, loudness?: number}} [opts]
     *   radius   — how far (m) this noise can be heard/reacted to (default 18)
     *   loudness — 0..1 relative intensity, passed through for enemies that
     *              want to scale their reaction (default 1)
     */
    this.emitNoise = (position, { radius = 18, loudness = 1 } = {}) => {
      for (const cb of this._noiseListeners) {
        cb({ position, radius, loudness });
      }
    };
