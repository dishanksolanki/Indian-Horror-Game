...
export class Engine {
  ...
  _updateInteractionFocus() {
    ...
    if (closest) {
      // prompt may be a static string or a function returning the current label,
      // so interactables (e.g. a drawer) can change their prompt based on state.
      const label = typeof closest.prompt === "function" ? closest.prompt() : closest.prompt;
